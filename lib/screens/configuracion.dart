...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _savePreferences() async {
    ...
    // DEBUG: Imprimimos qué guardamos
    debugPrint(
      "SettingsScreen: Guardando ubicación -> ${_storageLocation.name}",
    );
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                "Versión 12.0.12+12 - Lanzada en Junio 2026",
                style: TextStyle(
                  color: textColor.withOpacity(0.7),
                  fontSize: 14,
                ),
              ...
              const SizedBox(height: 4),
              // CAMBIO: Texto actualizado a OGG
              _buildBulletPoint("⚙️ Formatos: AAC, OGG y FLAC.", textColor),
              const SizedBox(height: 4),
              _buildBulletPoint(
                "✂️ Recortar y editar metadatos básicos.",
                textColor,
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // CAMBIO: Mostrar selector de Bitrate también para OGG (Opus)
          if (_selectedEncoder == AudioEncoder.aacLc ||
              _selectedEncoder == AudioEncoder.opus)
            _buildBitrateSelector(
              cardColor,
              textColor,
              subtitleColor,
      ...
      trailing: DropdownButton<AudioEncoder>(
        ...
        onChanged: (AudioEncoder? newValue) {
          if (newValue != null) {
            setState(() {
              _selectedEncoder = newValue;
              // CAMBIO: Ajuste de bitrate predeterminado para OGG
              _selectedBitRate = (newValue == AudioEncoder.aacLc)
                  ? 128000
                  : (newValue == AudioEncoder.opus ? 64000 : 0);
            });
            _notifyChanges();
          }
        },
        items: const [
          ...
          // CAMBIO: Reemplazado WAV por OGG
          DropdownMenuItem(value: AudioEncoder.opus, child: Text("OGG (.ogg)")),
          DropdownMenuItem(
            value: AudioEncoder.flac,
            child: Text("FLAC (.flac)"),
          ),
  ...
  Widget _buildBitrateSelector(
    ...
    // Si es AAC mostramos kbps normales, si es Opus (OGG) también es válido ajustar kbps
    String suffix = "kbps";
    ...
    return _buildSettingCard(
      ...
      icon: Icons.high_quality,
      title: "Tasa de Bits (Bitrate)",
      subtitle: "${_selectedBitRate ~/ 1000} $suffix",
      trailing: DropdownButton<int>(
        value: _selectedBitRate,
        dropdownColor: cardColor,
        style: TextStyle(color: textColor),
  ...
  Widget _buildStorageSelector(
    ...
    return _buildSettingCard(
      cardColor: cardColor,
      textColor: textColor,
      subtitleColor: subtitleText.codeUnits.isNotEmpty
          ? subtitleColor
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                onTap: () {
                  ...
                  Future.microtask(() async {
                    try {
                      if (!await appFolder.exists()) {
                        ...
                        debugPrint("SettingsScreen: Carpeta creada en SD");
                      }
                    } catch (e) {
                      debugPrint("SettingsScreen: Error creando carpeta: $e");
                    }
                  });
                  ...
                  // 3. Guardar preferencias (CRÍTICO)
                  _savePreferences();
